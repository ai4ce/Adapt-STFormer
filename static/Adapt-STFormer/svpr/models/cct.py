...
from .cct_utils.helpers import pe_check
...
class CCT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_seqpool = False
        if kwargs.get("use_all_tokens", False):
            self.use_seqpool = True
    ...
    def forward(self, x):  # seq_pool=False
        x = self.tokenizer(x)
        return self.classifier(x, self.use_seqpool)
    ...
    **kwargs,
    ...
    if pretrained:
        if arch in model_urls:
            ...
            if kwargs.get("abs_pos_embed", False):
                state_dict.pop("classifier.positional_emb")
