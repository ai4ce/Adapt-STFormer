...
class Attention(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_for_t:
            ...
            self.stride = 3
            self.s = 5 
            # self.s = 1 #NOTE: JUST FOR TESTING
        ...
        if self.rel_pos_embed:
            ...
            self.rel_pos_h = nn.Parameter(
                torch.zeros(spatial_dim, self.head_dim)
            )  # mvit2
    ...
    def forward(self, x):
        if self.use_for_t:
            ...
            x = x.view(B1, int(sqrt(N1)), int(sqrt(N1)), C).permute(
                0, 3, 1, 2
            )  # x1 [B,C,H,W]
            x = F.unfold(
                x.clone(), kernel_size=self.kernel_size, dilation=1, stride=self.stride
            )  # [B, C*6*6, L]
            _, _, L = x.size()
            x = x.permute(0, 2, 1).view(
                B1, L, C, self.kernel_size, self.kernel_size
            )  # [B,L,C,6,6]
            ...
            if (
                self.rel_pos_h is not None
                and self.rel_pos_w is not None
                and self.rel_pos_t is not None
            ):
...
class TransformerClassifier(Module):
    ...
    def forward(self, x, seq_pool=False):
        ...
        if (
            (self.positional_emb is not None)
            and (not self.rel_pos_temporal)
            and (not self.rel_pos_spatial)
        ):
