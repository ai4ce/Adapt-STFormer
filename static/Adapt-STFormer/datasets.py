...
class Dataset:
    ...
    def get_training_query_set(
        self, opt, margin=0.1, nNegSample=1000, use_regions=False
    ):
        structFile = join(self.struct_dir, self.train_mat_file)
        indsSplit = self.trainInds
...
def getSeqInds(idx, seqL, maxNum, minNum=0, retLenDiff=False):
    seqLOrig = seqL
    seqInds = np.arange(
        max(minNum, idx - seqL // 2), min(idx + seqL - seqL // 2, maxNum), 1
    )
    ...
    if seqInds[0] == minNum:
        seqInds = np.concatenate(
            [seqInds, np.arange(seqInds[-1] + 1, seqInds[-1] + 1 + lenDiff, 1)]
        )
    elif lenDiff > 0 and seqInds[-1] in range(maxNum - 1, maxNum):
        seqInds = np.concatenate(
            [np.arange(seqInds[0] - lenDiff, seqInds[0], 1), seqInds]
        )
    return seqInds
...
def print_db_concise(db):
    print("hello")
    [
        print("\033[1m" + k + "\033[0m", v[:10] if type(v) is list else v)
        for k, v in db._asdict().items()
    ]
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(
        ...
        if seqBounds[0] is None:
            self.seqBounds = np.array(
                [[0, len(self.images)] for _ in range(len(self.images))]
            )
        if not onlyDB:
            qImages = qDescs[indsSplit[1]]
            self.images = np.concatenate([self.images, qImages], 0)
            if seqBounds[0] is None:
                q_seqBounds = np.array(
                    [
                        [len(self.seqBounds), len(self.images)]
                        for _ in range(len(qImages))
                    ]
                )
        ...
        if self.filterBoundaryInds:
            ...
            self.dbStruct = self.dbStruct._replace(
                utmDb=self.dbStruct.utmDb[self.validInds_db],
                numDb=len(self.validInds_db),
                utmQ=self.dbStruct.utmQ[self.validInds_q],
                numQ=len(self.validInds_q),
            )
    ...
    def get_positives(self, retDists=False):
        ...
        if self.dbStruct.dataset == "nuscene_small" or self.dbStruct.dataset == "nuscene_large":
            self.positives = pickle.load(open(f"./structFiles/saved_positives/nuscene_positives_{self.dbStruct.dataset}.pkl", 'rb'))
            print("using pre-generated nuscene positives")
        elif self.positives is None:
            knn = NearestNeighbors(n_jobs=-1)
            knn.fit(self.dbStruct.utmDb)
            print("Using Localization Ra dius: ", self.dbStruct.posDistThr)
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(
        ...
        if seqBounds[0] is None:
            self.db_seqBounds = np.array(
                [[0, len(self.dbDescs)] for _ in range(len(self.dbDescs))]
            )
            self.q_seqBounds = np.array(
                [[0, len(self.qDescs)] for _ in range(len(self.qDescs))]
            )
        else:
        ...
        knn.fit(self.dbStruct.utmDb)  # changed
        ...
        print(
            "\n Queries within range ",
            len(self.queries),
            len(self.nontrivial_positives),
            "\n",
        )
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.cache, mode="r") as h5:
            ...
            negSample = np.sort(
                negSample
            )  # essential to order ascending, speeds up h5 by about double
        ...
        for negIndex in negIndices:
            ...
            negative = torch.tensor(
                self.dbDescs[getSeqInds(negIndex, self.seqL, sIdMax_n, sIdMin_n)]
            )
        ...
        # print("I am on this function call!")
